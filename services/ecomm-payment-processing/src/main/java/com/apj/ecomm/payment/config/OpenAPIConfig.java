package com.apj.ecomm.payment.config;
...
public class OpenAPIConfig {
	...
	@Bean
	GroupedOpenApi adminApi(@Value("${api.version}${admin.path}${payments.path}") final String path) {
		return getApiFor("admin", path);
	}

	@Bean
	GroupedOpenApi payApi(@Value("${api.version}${payments.path}") final String path) {
		return getApiFor("pay", path);
	}
