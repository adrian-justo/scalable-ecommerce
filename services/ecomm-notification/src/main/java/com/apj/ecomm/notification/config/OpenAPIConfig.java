...
public class OpenAPIConfig {
	...
	@Bean
	GroupedOpenApi adminApi(@Value("${api.version}${admin.path}${notifications.path}") final String path) {
		return getApiFor("admin", path);
	}

	@Bean
	GroupedOpenApi payApi(@Value("${api.version}${notifications.path}") final String path) {
		return getApiFor("notif", path);
	}
